...
from losses.my_loss_utils import visualize_pca, create_place_holder_dict
#end new


class PUALoss(nn.Module):

    def __init__(self, args_): #new: used to be args
        super().__init__()
        ...
        self.action_u_info = create_place_holder_dict(self.args.device)
        #end new
        ...
        elif args.time_prior == 'sep_sttrans':
            from models import MODELS
            temp_arg = self.model_args.copy()
            temp_arg['mean_pose']="uncertainty"
            self.model_U = MODELS['st_transformer'](temp_arg)
    ...
    def calc_sigma(self, y_true):
        ...
        if 'T' in self.args.tasks:
            ...
            elif self.args.time_prior == 'sep_sttrans':
                local_sigma = self.model_U(y_true)
                local_sigma = local_sigma["sigmas"]
                local_sigma = local_sigma.reshape(-1, 25, 32, 3)
                local_sigma = torch.norm(local_sigma, dim=-1) #/ 1.73205080757
