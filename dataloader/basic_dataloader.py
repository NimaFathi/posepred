    ...
    def __getitem__(self, index):
        ...
        for person_index in range(seq_persons_num):
            ...
            obs_pose = torch.tensor(
                [person_obs_pose_frames[i] for i in range(0, self.obs_frames_num, self.args.skip + 1)])
            ...
            if self.args.use_mask:
                ...
                obs_mask = torch.tensor(
                    [person_obs_mask_frames[i] for i in range(0, self.obs_frames_num, self.args.skip + 1)])
                person_future_mask_frames = seq.future_mask[person_index]
                true_mask = torch.tensor(
                    [person_future_mask_frames[i] for i in range(0, self.future_frames_num, self.args.skip + 1)])
