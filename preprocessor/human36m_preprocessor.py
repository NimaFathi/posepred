...
#
#                 skip_frame_between_sequences = total_frame_num * (self.skip_frame_num + 1)
#                 if self.use_video_once:
#                     skip_frame_between_sequences = positions.shape[0]
#                 if self.overlap_sequences:
#                     skip_frame_between_sequences = self.skip_frame_num + 1
#
#                 for i in range(0, positions.shape[0] - total_frame_num * (self.skip_frame_num + 1), skip_frame_between_sequences):
#                     video_data = {
#                         'observed_pose': list(),
#                         'future_pose': list(),
...
#                                 f'{os.path.basename(f).split(".cdf")[0]}_{i + j:05}')
#                         else:
...
#                         writer.write({
#                             'video_section': f'{subject}-{canonical_name}-{i // skip_frame_between_sequences}',
                ...
                for i in range(section_range):
                    ...
                    with jsonlines.open(os.path.join(self.output_dir, output_file_name), mode='a') as writer:
                        writer.write({
                            ...
                            # 'observed_quaternion_pose': video_data['observed_quaternion_pose'],
                            # 'future_quaternion_pose': video_data['future_quaternion_pose'],
                            'observed_pose': video_data['observed_expmap_pose'],
                            'future_pose': video_data['future_expmap_pose'],
                            'observed_image_path': video_data['observed_image_path'],
                            'future_image_path': video_data['future_image_path']
                        })
        self.save_meta_data(self.meta_data, self.output_dir, True, data_type)
