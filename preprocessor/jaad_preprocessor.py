...
class JAADPreprocessor(Processor):
    ...
    def normal(self, data_type='train'):
        ...
        if self.custom_name:
            ...
            output_file_name = f'{data_type}_{self.obs_frame_num}_{self.pred_frame_num}_{self.skip_frame_num}_JAAD.{DATA_FORMAT}'
    ...
    def __create_data(self, inpute_data, obs_frame_range, pred_frame_range, video_name, groups):
        data = []
        raw_data_obs = defaultdict(dict)
        raw_data_pred = defaultdict(dict)
        for ped_id, value in inpute_data.items():
            for frame_number, pose in value.items():
