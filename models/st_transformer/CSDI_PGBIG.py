...
class diff_CSDI(nn.Module):
    def __init__(self, args, inputdim, side_dim):
        ...
        assert self.diff_layers % self.stages == 0
        self.ch_per_st = self.diff_layers // self.stages
    ...
    def forward(self, x, cond_info):
        ...
        for i in range(1, self.stages + 1):
            x = torch.sum(skip[:i * self.ch_per_st], dim=0) / math.sqrt(i * self.ch_per_st)
            x = x.reshape(B, self.channels, K * L)
...
class CSDI_PGBIG_H36M(CSDI_base):
    def __init__(self, args):
        super(CSDI_PGBIG_H36M, self).__init__(args)
    ...
    def postprocess_data(self, batch, predicted):
        ...
        for i in range(len(predicted)):
            predicted[i] = predicted[i][:, :, self.Lo:]
            predicted[i] = predicted[i].permute(0, 2, 1)
            predicted[i] = self.postprocess(batch['observed_pose'], predicted[i])

        return {
            'pred_pose': predicted[-1],  # B, T, JC
            # 'preprocess': partial(self.preprocess, normal=False)
            'pred_stage': predicted
        }