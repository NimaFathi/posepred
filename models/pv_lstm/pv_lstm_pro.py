...
class PVLSTMPro(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # vel_encoder
        (hidden_vel, cell_vel) = self.vel_encoder(comp_vel.permute(1, 0, 2))
        hidden_vel = hidden_vel.squeeze(0)
        cell_vel = cell_vel.squeeze(0)

        # pose_encoder
        (hidden_pose, cell_pose) = self.pose_encoder(comp_pose.permute(1, 0, 2))
        hidden_pose = hidden_pose.squeeze(0)
        cell_pose = cell_pose.squeeze(0)
        ...
        # vel_decoder
        pred_vel2 = self.vel_decoder(comp_vel[:, -1, :], hidden_dec, cell_dec)
        if self.args.use_dct:
            pred_vel2 = torch.matmul(self.idct_future_vel.unsqueeze(0), pred_vel2)
        pred_pose2 = pose_from_vel(pred_vel2, comp_pose[..., -1, :])
        ...
        outputs = {'pred_pose': pred_pose, 'pred_vel': pred_vel, 'comp_pose': comp_pose, 'comp_vel': comp_vel,
                   'pred_pose2': pred_pose2, 'pred_vel2': pred_vel2, 'comp_pose_noise_only': comp_pose_noise_only}
