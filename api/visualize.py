...
from utils.others import dict_to_device, destabilize, get_matadata
...
def visualize(cfg: DictConfig):
    ...
    if 'observed' in showing:
        ...
        # print(pose.shape)
        if cfg.data.normalize:
            metadata = get_matadata(cfg.data.metadata_path)
            pose = destabilize(metadata, cfg.data.keypoint_dim, pose)
